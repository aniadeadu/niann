...
export const SearchUI = () => {
   ...
   useEffect(() => {
      if(navbar_search === false) {
         ...
         setNavbar_search_style (
            'text-white font-medium pl-12 pr-8 py-8 w-[45%]  right-[7rem] bg-transparent absolute top-0  transition-all duration-300 '
         )
      }
   ...
   const handleSearch = () => {
      const searched_product_link = search_product_links(word, all_products_links); 
         ...
         searched_product_link.map((product_link) => {
            ...
            return (
               <div className="flex gap-1">
                  ...
                  <div className="text-white cursor-pointer" onClick={() => {navigate(product_link.Link)
                  }}>
                     {product_link.Name}
                  </div>
               </div>
      ...
      setSearched_image(
         searched_product.map((product) => {
            return (
               <div className="basis-[25%] cursor-pointer" onClick={() => {
                  navigate(
                     (product.id.toString().includes('312150')) ?
                     `/shopping/women/wears/${product.alt}/${product.id}` : 
                     (product.id.toString().includes('312151')) ?
                     `/shopping/men/wears/${product.alt}/${product.id}` : 
                  ...
                  <img 
                     src={product.image} 
                     alt={product.alt} 
                     className="object-contain h-auto rounded-md cursor-pointer"
                  />
               </div>
            )
         })
   ...
   return (navbar_search &&
      <ModalUI
         navbar_search_style={'  h-[30rem]  w-[45%]  scroll-smooth ' + navbar_search_style}
      >
         <div
            className="w-full pl-6 pr-8"
         >
            <InputUI
               className={'bg-transparent w-[85%] border-b py-1 px-1 outline-none border-b-[rgba(255,255,255,.95)] text-white'}
               type={ 'search' }
